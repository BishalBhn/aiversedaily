...
function renderPrerender(c) {
  ...
  // Blogs link to their on-site article pages (/blog/<slug>).
  if (c.blogs) {
    var blogCards = (c.blogs.items || []).map(function (it) {
      var url = "/blog/" + (it.slug || slugify(it.title));
      return "<article><h3><a href=\"" + esc(url) + "\">" + esc(it.title) + "</a></h3>" +
        "<p>" + esc(it.excerpt || "") + ' <a href="' + esc(url) + '">Read more</a></p></article>';
    }).join("\n");
    parts.push("<section>" + headBlock(null, c.blogs.heading, c.blogs.sub) + blogCards + "</section>");
  }
